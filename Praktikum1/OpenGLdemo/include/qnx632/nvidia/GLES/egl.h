...
#define EGL_MATCH_NATIVE_PIXMAP     0x3041    /* Pseudo-attribute 
                                                 (not queryable) */
...
/* EGL 1.2 multi API support */
GLAPI EGLBoolean APIENTRY eglBindAPI (EGLenum api);
GLAPI EGLint APIENTRY eglQueryAPI (void);

GLAPI EGLBoolean APIENTRY eglWaitClient (void);

/* EGL 1.2 thread resource allocation control API */
GLAPI EGLBoolean APIENTRY eglReleaseThread (void);

/* EGL 1.2 create-pbuffer-from-client-buffer extension */
GLAPI EGLSurface APIENTRY eglCreatePbufferFromClientBuffer (EGLDisplay dpy, EGLenum buftype, EGLClientBuffer buffer, EGLConfig config, const EGLint *attrib_lib);
